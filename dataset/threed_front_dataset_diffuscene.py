...
class ThreedFrontDataset(data.Dataset):
    ...
    def post_process(self, samples: np.ndarray):
        ...
        # # descale class labels
        # class_labels = samples[:, :class_label_dim]
        # descaled_class_labels = (class_labels + 1) / 2
        # # # delete empty class labels
        # # descaled_class_labels = descaled_class_labels[descaled_class_labels[:, -1] != 1]

        # # descale center
        # center = samples[:, class_label_dim:class_label_dim + center_dim]
        # descaled_centers = self.descale(center, *center_bounds)

        # # descale size
        # size = samples[:, class_label_dim + center_dim:class_label_dim + center_dim + size_dim]
        # descaled_sizes = self.descale(size, *size_bounds)

        # # cvt cos,sin to angle
        # cos_sin_angle = samples[:, class_label_dim + center_dim + size_dim:class_label_dim + center_dim + size_dim +
        #                         angle_dim]
        # angles = np.arctan2(cos_sin_angle[:, 1:2], cos_sin_angle[:, 0:1])

        # # concatenate
        # descaled_samples = np.concatenate([descaled_class_labels, descaled_centers, descaled_sizes, angles], axis=-1)
