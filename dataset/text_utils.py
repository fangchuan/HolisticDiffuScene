...
def compute_rel(box1, box2):
    ...
    d = center1 - center2
    theta = math.atan2(d[1], d[0])  # range -pi to pi

    distance = (d[1]**2 + d[0]**2)**0.5

    # "on" relationship
    p = None
    if center1[0] >= ox0 and center1[0] <= ox1:
        if center1[1] >= oy0 and center1[1] <= oy1:
            delta1 = center1[2] - center2[2]
            delta2 = (sz1 - sz0 + oz1 - oz0) / 2
    ...
    # eliminate relation in z axis now
    if abs(d[2]) > 0.5:
        return p, distance
    ...
    if sx0 < ox0 and sx1 > ox1 and sy0 < oy0 and sy1 > oy1:
        ...
        p = 'inside'
    # 60 degree intervals along each direction, forward direction is negative y-axis
...
def compute_rel_3dfront(box1, box2):
    center1 = np.array([(box1[0] + box1[3]) / 2, (box1[1] + box1[4]) / 2, (box1[2] + box1[5]) / 2])
    center2 = np.array([(box2[0] + box2[3]) / 2, (box2[1] + box2[4]) / 2, (box2[2] + box2[5]) / 2])
