...
from .threed_front import ThreedFront
from .threed_front_scene import BaseThreedFutureModel
...
class DatasetDecoratorBase(Dataset):
    ...
    def __init__(self, dataset: ThreedFront):
...
class BoxOrderedDataset(DatasetDecoratorBase):
    ...
    def __init__(self, dataset: ThreedFront, box_ordering=None):
        super().__init__(dataset)
        self.box_ordering = box_ordering

    @lru_cache(maxsize=16)
    def _get_boxes(self, scene_idx) -> List[BaseThreedFutureModel]:
        scene = self._dataset[scene_idx]
        if self.box_ordering is None:
            extras = [e for e in scene.extras if e.label in ["wall", "door", "window"]]
            return scene.bboxes + extras
...
class DatasetCollection(DatasetDecoratorBase):
    ...
    def collate_fn(samples):
        ...
        for sample in samples:
            rooms_text.append(sample['room_text'])
            del sample['room_text']
        ...
        sample_params = {}
        sample_params.update({k: np.stack([sample[k] for sample in samples], axis=0) for k in (key_set - padding_keys)})
            ...
            k:
                np.stack([
                    ...
                    for sample in samples
                         ...
                         axis=0) for k in padding_keys
        })
        sample_params["lengths"] = np.array([sample["length"] for sample in samples])

        # Make torch tensors from the numpy tensors
        torch_sample = {k: torch.from_numpy(sample_params[k]).float() for k in sample_params}

        torch_sample.update({k: torch_sample[k][:, None] for k in torch_sample.keys() if "_tr" in k})

        torch_sample.update({'room_text': rooms_text})
...
class Autoregressive(DatasetDecoratorBase):
    ...
    def __getitem__(self, idx):
        ...
        for k, v in sample_params.items():
            if k == "room_layout" or k == "length" or k == 'room_text':
                pass
            elif k == "class_labels":
        ...
        for k, v in sample_params.items():
            if k == "room_layout" or k == "length":
                ...
                sample_params_target[k] = np.vstack(
                    [new_class_labels, np.tile(end_label[None, :], [max_length - L, 1])]).astype(np.float32) * 2.0 - 1.0

            else:
                p = np.copy(v)
                # Set the attributes to for the end symbol
                L, C = p.shape
                sample_params_target[k] = np.vstack([p, np.tile(np.zeros(C)[None, :],
                                                                [max_length - L, 1])]).astype(np.float32)
...
def dataset_encoding_factory(name, dataset, augmentations=None, box_ordering=None):
    ...
    #       though.
    if "cached" in name:
        dataset_collection = OrderedDataset(CachedDatasetCollection(dataset),
        ...
        dataset_collection = DatasetCollection(room_layout, class_labels, translations, sizes, angles)
    ...
    # for autoregressive model
        ...
        if "eval" in name:
            ...
            dataset_collection = Permutation(dataset_collection, ["class_labels", "translations", "sizes", "angles"])
            return AutoregressiveWOCM(dataset_collection)
        else:
            raise NotImplementedError()
