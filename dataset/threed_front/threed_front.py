...
from .threed_front_scene import Room, BaseThreedFutureModel
...
class ThreedFront(BaseDataset):
    ...
    def __init__(self, scenes: List[Room], bounds=None):
        super().__init__(scenes)
        # verify that all scenes are of type Room
    ...
    def _compute_bounds(self):
        ...
        for s in self.scenes:
            ...
            for e in extras:
                ...
                _centroid_min = np.minimum(centroid, _centroid_min)
                _centroid_max = np.maximum(centroid, _centroid_max)
                _size_min = np.minimum(self._size(e), _size_min)
                _size_max = np.maximum(self._size(e), _size_max)
                _angle_min = np.minimum(e.z_angle, _angle_min)
                _angle_max = np.maximum(e.z_angle, _angle_max)
    ...
    def max_furniture_length(self):
        if self._max_furniture_length is None:
            ...
            if 'bed' in _room_types:
                ...
                self._max_furniture_length = 21
            elif 'dining' in _room_types:
                self._max_furniture_length = 21
            elif 'library' in _room_types:
                self._max_furniture_length = 11
    ...
    def from_dataset_directory(cls,
        ...
        scenes = parse_threed_front_scenes(dataset_directory, path_to_model_info, path_to_models,
        ...
        for s in map(filter_fn, scenes):
            if not s:
                continue
            # print(f'*********************************** preprocessing {s.uid} ***********************************')

            s.extras = s.merge_extras_with_walls_doors_windows()
            filtered_scenes.append(s)

        return cls(filtered_scenes, bounds)


class CachedRoom(object):

    def __init__(self, scene_id, room_layout, floor_plan_vertices, floor_plan_faces, floor_plan_centroid, class_labels,
                 translations, sizes, angles, image_path):
...
class CachedThreedFront(ThreedFront):
    ...
    def __init__(self, base_dir, config, scene_ids):
        ...
        self._tags = sorted([oi for oi in os.listdir(self._base_dir) if oi.split("_")[1] in scene_ids])
        self._path_to_rooms = sorted([os.path.join(self._base_dir, pi, "boxes.npz") for pi in self._tags])
        rendered_scene = "rendered_scene_256.png"
        path_to_rendered_scene = os.path.join(self._base_dir, self._tags[0], rendered_scene)
    ...
    def _get_room_layout(self, room_layout):
        ...
        img = img.resize(tuple(map(int, self.config["room_layout_size"].split(","))), resample=Image.BILINEAR)
    ...
    def __getitem__(self, i):
        ...
        return CachedRoom(scene_id=D["scene_id"],
    ...
    def __str__(self):
        return "Dataset contains {} scenes with {} discrete types".format(len(self), self.n_object_types)
